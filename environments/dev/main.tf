...
## 🚚 State Migration & Zero-Downtime Refactoring (`moved` Blocks)

A common challenge in infrastructure engineering is refactoring live systems without destroying existing assets. In a naive migration, splitting resources into modules causes Terraform to think the old resource was deleted and a new one needs to be created from scratch.

To address this, this project implements a declarative **`moved` block** inside `environments/dev/main.tf`:
...
### Why this is a Crucial Best Practice:
*   **Prevents Data Loss:** It instructs Terraform to gracefully map the existing resource address in the state file (`.tfstate`) to the new nested module path instead of tearing down the server.
*   **Zero Downtime:** The virtual machine remains completely untouched during the transition, preventing service disruptions.
*   **Git-Ops Compliant:** It replaces the legacy, manual, and error-prone `terraform state mv` CLI commands with tracked code, allowing team-wide state refactoring via standard Pull Requests.
